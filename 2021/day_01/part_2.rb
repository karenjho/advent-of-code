...
def count_of_sliding_depth_increases(measurements)
  last_window_index = measurements.length
  measurements.each_with_index.reduce(0) do |count, (_, i)|
    next count unless i > 2 && i <= last_window_index

    previous_window_sum = measurements[(i - 3)..(i - 1)].sum(&:to_i)
    current_window_sum = measurements[(i - 2)..i].sum(&:to_i)
    count += 1 if current_window_sum > previous_window_sum
    count
  end
end

input = File.read('input.txt').split("\n")
puts count_of_sliding_depth_increases(input)
