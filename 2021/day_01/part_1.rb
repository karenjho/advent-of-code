...
def count_of_depth_increases(measurements)
  measurements.each_with_index.reduce(0) do |count, (m, i)|
    count += 1 if measurements[i - 1] && measurements[i - 1].to_i < m.to_i
    count
  end
end

input = File.read('input.txt').split("\n")
puts count_of_depth_increases(input)
